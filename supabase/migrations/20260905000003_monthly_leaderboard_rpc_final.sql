...
BEGIN
    ...
    session_award_winners AS (
        WITH ranked AS (
            SELECT
                ...
                pv.candidate_id,
                COUNT(*) as votes,
                ROW_NUMBER() OVER (PARTITION BY pv.session_id, pv.award_type ORDER BY COUNT(*) DESC, pv.candidate_id ASC) as rnk
            FROM poll_votes pv
    ...
    SELECT
        ...
        SUM(CASE WHEN e.event_type = 'GOAL' THEN 1 ELSE 0 END)::BIGINT as goals,
        SUM(CASE WHEN e.event_type = 'ASSIST' THEN 1 ELSE 0 END)::BIGINT as assists,
