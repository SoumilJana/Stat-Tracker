ALTER TABLE seasons 
ADD COLUMN motm_id UUID REFERENCES profiles(id) ON DELETE SET NULL,
ADD COLUMN motm_awards INT;
...
$$
  ...
  SELECT
    ...
    COALESCE((SELECT SUM(sts.matches_played - sts.wins) FROM sp sp2 JOIN session_team_stats sts ON sts.session_id = sp2.session_id AND sts.team_id = sp2.team_id WHERE sp2.player_id = ap.player_id), 0)::BIGINT AS losses,
    0::BIGINT AS draws,
    ...
    COALESCE((SELECT COUNT(*) FROM aw WHERE aw.candidate_id = ap.player_id AND aw.award_type = 'MOTM'), 0)::BIGINT AS motm_awards
  FROM ap;
$$;
